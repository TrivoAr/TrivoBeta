      ...
      {items.length > 0 ? (
        items.map((item) => (
          <DashboardCard
            ...
            date={item.fecha ? dayjs(item.fecha).format("DD/MM/YYYY") : undefined}
            time={item.hora}
            price={item.precio}
            type={item.nombre_academia ? "academia" : "salida"}
            showActions={showActions}
            onClick={() => onView(item._id)}
            onEdit={onEdit ? () => onEdit(item._id) : undefined}
            onDelete={onDelete ? () => onDelete(item._id) : undefined}
            onViewMembers={onViewMembers ? () => onViewMembers(item._id) : undefined}
          />
        ))
      ) : (
...
export const SalidasSocialesSection: React.FC<SalidasSocialesSectionProps> = (props) => (
  <DashboardSection
    {...props}
    items={props.salidas}
...
export const TeamSocialesSection: React.FC<TeamSocialesSectionProps> = (props) => (
  <DashboardSection
    {...props}
    items={props.teams}
    ...
    <div className="space-y-4">
      ...
      {miMatchSalidas.map((salida) => (
        <DashboardCard
          ...
          date={salida.fecha ? dayjs(salida.fecha).format("DD/MM/YYYY") : undefined}
          time={salida.hora}
          price={salida.precio}
          teacher={salida.creador_id ? `${salida.creador_id.firstname} ${salida.creador_id.lastname}` : undefined}
          type="salida"
 ...
 */
interface FavoritoItem extends BaseItem {
  nombre?: string;
  nombre_academia?: string;
  deporte?: string;
  tipo_disciplina?: string;
...
export interface FavoritosSectionProps {
  ...
  onToggleFavorite: (tipo: 'academias' | 'sociales' | 'teamsocial', id: string) => void;
  onExploreEvents: () => void;
}
        ...
        <div className="space-y-4">
          ...
          {favoritosAcademias.map((academia) => (
            <DashboardCard
              ...
              showActions={true}
              isFavorite={true}
              onToggleFavorite={() => onToggleFavorite('academias', academia._id)}
            />
          ))}
        </div>
      )}
