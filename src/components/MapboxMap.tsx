  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      try {
        ...
        console.log('Token value:', process.env.NEXT_PUBLIC_MAPBOX_TOKEN ? 'TOKEN_SET' : 'TOKEN_NOT_SET');
        ...
        map.current.on('load', () => {
          ...
          marker.current.on('dragend', async () => {
            ...
            try {
              const response = await fetch(`/api/search/reverse?lat=${coords.lat}&lon=${coords.lng}`);
              ...
              onLocationSelectRef.current?.(coords, `${coords.lat.toFixed(6)}, ${coords.lng.toFixed(6)}`);
            }
          });
