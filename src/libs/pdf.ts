...
export async function buildQrPdf(title: string, pngBase64: string, ticketInfo?: TicketInfo): Promise<string> {
  ...
  return await new Promise<string>((resolve) => {
    ...
    // Event name
    const eventName = ticketInfo?.eventName || title.replace('QR de acceso — ', '');
    doc.fontSize(16)
       .font('Helvetica-Bold')
       .fillColor('#C95100')
    ...
    if (ticketInfo) {
      ...
      if (ticketInfo.date || ticketInfo.time) {
        const dateTime = [ticketInfo.date, ticketInfo.time].filter(Boolean).join(' - ');
    ...
    // QR background
    doc.rect(qrX - 15, currentY - 15, imgW + 30, imgW + 30)
