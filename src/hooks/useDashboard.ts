...
export function useMyAcademias() {
  return useQuery({
    ...
    queryFn: async (): Promise<Academia[]> => {
      ...
      const filteredAcademias = Array.isArray(data) ? data.filter((academia: Academia) => 
...
export function useMyMatches() {
  return useQuery({
    queryKey: ['my-matches'],
    queryFn: async (): Promise<{salidas: SalidaSocialMatch[], teams: TeamSocial[]}> => {
      ...
      // Procesar salidas
      const salidas = Array.isArray(salidasData) ? salidasData.map((salida: any) => ({
        ...salida,
        imagen: salida.imagen || `https://ui-avatars.com/api/?name=${encodeURIComponent(salida.nombre)}&background=C95100&color=fff&size=310x115`
      })) : [];
...
export function useMyFavorites(userId?: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<{academias: Academia[], salidas: SalidaSocialMatch[], teams: TeamSocial[]}> => {
...
export function useToggleFavorite() {
  ...
  return useMutation({
    mutationFn: async ({ tipo, id }: { tipo: 'academias' | 'sociales' | 'teamsocial', id: string }) => {
