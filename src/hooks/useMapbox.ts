...
export const useMapbox = (): UseMapboxReturn => {
  ...
  const [clickHandler, setClickHandler] = useState<((e: any) => void) | null>(
    null
  );

  // Inicializar mapa
  const initializeMap = useCallback(
    async (container: HTMLElement, config: MapboxConfig) => {
      try {
        setLoading(true);
        setError(null);
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          mapInstance.on("error", (e: any) => {
            reject(
              new Error(
                `Error al cargar el mapa: ${e.error?.message || "Desconocido"}`
              )
            );
          });
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Error desconocido al inicializar el mapa";
  ...
  const onMapClick = useCallback(
    (callback: (coordinates: [number, number]) => void) => {
      if (!map) return;
...
export const useStravaRoute = () => {
  ...
  const addStravaRoute = useCallback(
    (mapbox: UseMapboxReturn, routeData: any, layerId = "strava-route") => {
      ...
      try {
        ...
        if (routeData.coordinates && routeData.coordinates.length > 0) {
          ...
          const bounds: [[number, number], [number, number]] = [
            [
              Math.min(
                ...coordinates.map((coord: [number, number]) => coord[0])
              ),
              Math.min(
                ...coordinates.map((coord: [number, number]) => coord[1])
              ),
            ],
            [
              Math.max(
  ...
  const removeStravaRoute = useCallback(
    (mapbox: UseMapboxReturn, layerId = "strava-route") => {
      mapbox.removeLayer(layerId);
      setRouteLayer(null);
    },
    []
...
export const useMapboxGeocoding = (accessToken?: string) => {
  ...
  const search = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error en geocodificación";
        setError(errorMessage);
        setLoading(false);
        return [];
      }
  ...
  const reverseGeocode = useCallback(
    async (coordinates: [number, number]): Promise<any | null> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const response = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${coordinates[0]},${coordinates[1]}.json?${params}`
        );

        if (!response.ok) {
          throw new Error(
            `Error en geocodificación reversa: ${response.status}`
          );
        }
