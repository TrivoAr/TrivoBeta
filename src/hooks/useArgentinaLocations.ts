...
const ARGENTINA_LOCATIONS: Province[] = [
  {
    ...
    name: "Tucumán",
    localities: [
      { id: "san_miguel", name: "San Miguel de Tucumán", province_id: "tucuman" },
      { id: "yerba_buena", name: "Yerba Buena", province_id: "tucuman" },
      { id: "tafi_viejo", name: "Tafí Viejo", province_id: "tucuman" },
      { id: "banda_rio_sali", name: "Banda del Río Salí", province_id: "tucuman" },
  ...
  },
  {
    ...
    name: "Buenos Aires",
    localities: [
      { id: "caba", name: "Ciudad Autónoma de Buenos Aires", province_id: "buenos_aires" },
      { id: "la_plata", name: "La Plata", province_id: "buenos_aires" },
      { id: "mar_del_plata", name: "Mar del Plata", province_id: "buenos_aires" },
      ...
      { id: "vicente_lopez", name: "Vicente López", province_id: "buenos_aires" },
  ...
  },
  {
    ...
    name: "Córdoba",
    localities: [
      { id: "cordoba_capital", name: "Córdoba Capital", province_id: "cordoba" },
      { id: "villa_carlos_paz", name: "Villa Carlos Paz", province_id: "cordoba" },
  ...
  },
  {
    ...
    name: "Santa Fe",
    localities: [
      { id: "santa_fe_capital", name: "Santa Fe Capital", province_id: "santa_fe" },
      ...
      { id: "villa_gobernador_galvez", name: "Villa Gobernador Gálvez", province_id: "santa_fe" },
  ...
  },
  {
    ...
    name: "Mendoza",
    localities: [
      { id: "mendoza_capital", name: "Mendoza Capital", province_id: "mendoza" },
...
export function useLocationFromCoords() {
  return useMutation({
    mutationFn: async (coords: { lat: number; lng: number }): Promise<LocationData> => {
      try {
...
function mapToKnownLocality(cityName: string, provinceId: string | null): string | null {
  ...
  const locality = province.localities.find(loc => {
    ...
    return normalizedLocality.includes(normalizedCity) || normalizedCity.includes(normalizedLocality);
  });
...
function detectProvinceByCoords(coords: { lat: number; lng: number }): string | null {
  const { lat, lng } = coords;
  
  // Rangos aproximados de coordenadas para cada provincia
  const provinceRanges: Record<string, { latMin: number; latMax: number; lngMin: number; lngMax: number }> = {
    tucuman: { latMin: -27.8, latMax: -26.0, lngMin: -66.5, lngMax: -64.5 },
    buenos_aires: { latMin: -42.0, latMax: -33.0, lngMin: -63.0, lngMax: -56.0 },
...
export function useSearchLocalities(searchTerm: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Array<{ province: string; locality: Locality }>> => {
