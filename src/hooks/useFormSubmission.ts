...
export interface UseFormSubmissionReturn {
  ...
  error: string | null;
  success: boolean;
  submitForm: (url: string, data: any, options?: FormSubmissionOptions) => Promise<any>;
  clearError: () => void;
  clearSuccess: () => void;
  reset: () => void;
}
...
export const useFormSubmission = (): UseFormSubmissionReturn => {
  ...
  const submitForm = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error desconocido al enviar formulario';
...
export const useSocialEventSubmission = () => {
  ...
  const createSocialEvent = useCallback(async (eventData: any) => {
    // Validación específica para eventos sociales
    const validateEventData = (data: any): boolean | string => {
      if (!data.nombreSalida?.trim()) return 'El nombre del evento es requerido';
      ...
      if (data.precio && isNaN(parseFloat(data.precio))) return 'El precio debe ser un número válido';
      if (data.limitePersonas && (!Number.isInteger(data.limitePersonas) || data.limitePersonas < 1)) {
  ...
  const updateSocialEvent = useCallback(async (eventId: string, eventData: any) => {
  ...
  const deleteSocialEvent = useCallback(async (eventId: string) => {
    return baseSubmission.submitForm(`/api/social/${eventId}`, {}, {
      method: 'DELETE',
      onSuccess: () => {
...
export const useFileSubmission = () => {
  ...
  const uploadMultipleFiles = useCallback(async (
    ...
    const uploadPromises = files.map(file => uploadFile(file, url, options));
 ...
 */
export const useRealTimeValidation = (validationRules: Record<string, (value: any) => boolean | string>) => {
  ...
  const validateField = useCallback((fieldName: string, value: any): boolean => {
    ...
    setErrors(current => {
      ...
      if (result === true) {
        ...
        newErrors[fieldName] = typeof result === 'string' ? result : 'Campo inválido';
      }
