...
import { useRouter } from "next/navigation";
import TopContainer from "@/components/TopContainer";
import DashboardCard from "@/components/DashboardCard"; // la que armamos recién
import Skeleton from "react-loading-skeleton";
import "react-loading-skeleton/dist/skeleton.css";
...
export default function DashboardPage() {
  ...
  const [selectedLocalidad, setSelectedLocalidad] = useState("San Miguel de Tucumán");

  // Estados
  const [loading, setLoading] = useState(true);
  const [salidaSocial, setSalidaSocial] = useState<any[]>([]);
  const [salidaTeamSocial, setSalidaTeamSocial] = useState<any[]>([]);
  const [miMatch, setMiMatch] = useState<any[]>([]);
  const [miMatchTeamSocial, setMiMatchTeamSocial] = useState<any[]>([]);
  const [salidasFavoritas, setSalidasFavoritas] = useState<any[]>([]);
  const [teamSocialFavoritos, setTeamSocialFavoritos] = useState<any[]>([]);
  const [academiasFavoritas, setAcademiasFavoritas] = useState<any[]>([]);
  const [academia, setAcademia] = useState<any | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 🔹 Simulación: reemplazar con tus endpoints reales
        const [salidas, teams, matchSalidas, matchTeams, favSalidas, favTeams, favAcademias] =
          await Promise.all([
            fetch("/api/social").then((r) => r.json()),
            fetch("/api/team-social").then((r) => r.json()),
            fetch("/api/social/unirse/estado").then((r) => r.json()),
            fetch("/api/team-social/mis").then((r) => r.json()),
            fetch("/api/favoritos/salidas").then((r) => r.json()),
            fetch("/api/favoritos/team-social").then((r) => r.json()),
            fetch("/api/favoritos/academias").then((r) => r.json()),
          ]);

        setSalidaSocial(salidas);
        setSalidaTeamSocial(teams);
        setMiMatch(matchSalidas.salidas ?? []);
        setMiMatchTeamSocial(matchTeams ?? []);
        setSalidasFavoritas(favSalidas ?? []);
        setTeamSocialFavoritos(favTeams ?? []);
        setAcademiasFavoritas(favAcademias ?? []);
      } catch (err) {
  ...
  if (status === "loading") return <p>Cargando...</p>;
  if (!session) return <p>No estás autenticado. Iniciá sesión.</p>;

  // === RENDER ===
  return (
    <main className="bg-[#FEFBF9] min-h-screen px-4 py-6 space-y-6 w-[390px] mx-auto">
      {/* Top */}
      <TopContainer
        selectedLocalidad={selectedLocalidad}
        setSelectedLocalidad={setSelectedLocalidad}
      ...
      <div className="flex space-x-3 justify-center overflow-x-auto pb-2 scrollbar-hide">
        {categories.map((cat) => (
          <button
            key={cat.label}
            onClick={() => setSelectedCategory(cat.label)}
            className={`flex-shrink-0 w-[110px] h-[36px] rounded-[15px] border shadow-sm text-sm font-medium ${
              selectedCategory === cat.label
        ...
        <>
          ...
          <Section title="Mis salidas">
            {loading ? (
              ...
              salidaSocial.map((event) => (
                <DashboardCard key={event._id} event={{ ...event, isOwner: true }} />
              ))
            ) : (
              <Empty msg="No tenés salidas creadas" action="/social/crear" />
            )}
          </Section>

          {/* Mis social team */}
          <Section title="Mis social team">
            {loading ? (
              <SkeletonCards />
            ) : salidaTeamSocial.length > 0 ? (
              salidaTeamSocial.map((event) => (
              ...
              <Empty msg="No tenés teams creados" action="/team-social/crear" />
            )}
          </Section>

          {/* Mi grupo principal (academia) */}
          {session.user.role === "dueño de academia" && (
            <Section title="Mi grupo principal">
              {academia ? (
                <DashboardCard event={academia} />
              ) : (
                <Empty msg="No tenés academias" action="/academias/crear" />
              )}
            </Section>
          )}
        </>
      ...
      {selectedCategory === "Mis match" && (
        <>
          {/* Match salidas */}
          <Section title="Match salidas">
            {loading ? (
              <SkeletonCards />
            ) : miMatch.length > 0 ? (
              miMatch.map((event) => <DashboardCard key={event._id} event={event} />)
            ) : (
              <Empty msg="No tenés match en salidas" />
            )}
          </Section>

          {/* Match social team */}
          <Section title="Match social team">
            {loading ? (
              <SkeletonCards />
            ) : miMatchTeamSocial.length > 0 ? (
              miMatchTeamSocial.map((event) => <DashboardCard key={event._id} event={event} />)
            ) : (
              <Empty msg="No tenés match en teams" />
            )}
          </Section>
        </>
      )}
      ...
      {selectedCategory === "Mis favoritos" && (
        <>
          {/* Salidas favoritas */}
          <Section title="Salidas favoritas">
            {loading ? (
              <SkeletonCards />
            ) : salidasFavoritas.length > 0 ? (
              salidasFavoritas.map((event) => <DashboardCard key={event._id} event={event} />)
            ) : (
              <Empty msg="No tenés salidas favoritas" />
            )}
          </Section>

          {/* Team favoritos */}
          <Section title="Teams favoritos">
            {loading ? (
              <SkeletonCards />
            ) : teamSocialFavoritos.length > 0 ? (
              teamSocialFavoritos.map((event) => <DashboardCard key={event._id} event={event} />)
            ) : (
              <Empty msg="No tenés teams favoritos" />
            )}
          </Section>

          {/* Academias favoritas */}
          <Section title="Academias favoritas">
            {loading ? (
              <SkeletonCards />
            ) : academiasFavoritas.length > 0 ? (
              academiasFavoritas.map((a) => <DashboardCard key={a._id} event={a} />)
            ) : (
              <Empty msg="No tenés academias favoritas" />
            )}
          </Section>
        </>
      )}
...
function Empty({ msg, action }: { msg: string; action?: string }) {
  return (
    <p className="text-slate-500 text-sm">
      {msg}{" "}
      {action && (
        <a href={action} className="text-orange-500 underline">
          Crear aquí
        </a>
      )}
    </p>
