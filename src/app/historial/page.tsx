...
export default function HistorialPagos() {
  ...
  const id_academia =
    typeof window !== "undefined" ? localStorage.getItem("academia_id") : null;
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const alumnosData: Alumno[] = alumnosRes.data.miembros.map(
          (miembro: any) => ({
    ...
    <div className="w-[380px] p-4 max-w-lg mx-auto">
      <div className="w-full max-w-lg mx-auto bg-gray-100 p-4 rounded-lg shadow-md">
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <button
            className="fixed bottom-20 right-4 p-4 rounded-full bg-white/20 backdrop-blur-md hover:bg-white/30 shadow-lg transition-transform transform hover:scale-110"
                ...
                {pagosPorGrupo[grupoId].map((pago) => (
                  ...
                  >
                    <div>
                      ...
                      <p
                        className={`text-sm ${pago.estado === "aprobado" ? "text-green-500" : "text-red-500"}`}
                      >
                        {pago.estado}
                      </p>
                    </div>
                    <div className="text-right">
                      <p
                        className={`font-semibold ${session?.user.rol === "dueño de academia" ? "text-green-500" : "text-red-500"}`}
                      >
