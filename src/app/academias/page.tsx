  ...
  const filtered = academias.filter((a) => {
    const matchesDiscipline =
      selectedDiscipline === "otros"
        ? !disciplines.some(
            (d) => d.key !== "otros" && d.key === a.tipo_disciplina
          )
        : a.tipo_disciplina === selectedDiscipline;

    const matchesSearch = a.nombre_academia
      .toLowerCase()
    ...
    <main className="bg-background min-h-screen text-foreground px-4 py-6 space-y-6 w-[390px] mx-auto">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">
          ...
          {selectedDiscipline.charAt(0).toUpperCase() +
            selectedDiscipline.slice(1)}
        </h2>
        ...
        <div className="flex space-x-4 pb-4">
          {filtered.length > 0 ? (
            ...
            <p className="text-sm text-gray-500 self-center">
              No hay academias para esta disciplina.
            </p>
          )}
        </div>
      </div>
