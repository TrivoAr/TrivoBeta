...
export default function EditarSalida({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (salidaQuery.data) {
      ...
      // Cargar imágenes existentes (priorizar array de imagenes)
      if (salidaQuery.data.imagenes && Array.isArray(salidaQuery.data.imagenes) && salidaQuery.data.imagenes.length > 0) {
        setExistingImageUrls(salidaQuery.data.imagenes);
        setPreviewUrl(salidaQuery.data.imagenes[0]);
      } else if (salidaQuery.data.imagen) {
        setExistingImageUrls([salidaQuery.data.imagen]);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Agregar nuevas imágenes a las existentes (máximo 5 total)
    const totalImages = existingImageUrls.length + previewUrls.length + filesArray.length;
    if (totalImages > 5) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Subir nuevas imágenes a Firebase
    ...
    const payload = {
      ...localSalida,
      imagen: allImageUrls.length > 0 ? allImageUrls[0] : "", // Primera imagen para compatibilidad
      ...
      >
        ...
        <label className="block mb-3">
          <div className="flex items-center justify-between mb-2">
            <span className="block text-sm">Imágenes {(existingImageUrls.length + previewUrls.length) > 0 && `(${existingImageUrls.length + previewUrls.length})`}</span>
              ...
              {previewUrls.map((url, index) => (
                <div key={`new-${index}`} className="relative group">
                  <img
                    src={url}
                    alt={`Nueva ${index + 1}`}
                    className="w-full h-24 object-cover rounded-md border border-green-500"
                  />
                  <button
                    type="button"
          ...
          {(existingImageUrls.length + previewUrls.length) < 5 && (
            <div className="w-full h-32 bg-white border-2 border-dashed border-gray-300 rounded-md flex items-center justify-center relative overflow-hidden hover:border-orange-500 transition-colors">
              <input
                type="file"
                accept="image/*"
                multiple
              ...
              <div className="text-center z-0 pointer-events-none">
                ...
                <span className="text-sm text-gray-500">
                  {(existingImageUrls.length + previewUrls.length) === 0 ? 'Seleccionar imágenes' : 'Agregar más imágenes'}
                </span>
              </div>
            </div>
          )}
