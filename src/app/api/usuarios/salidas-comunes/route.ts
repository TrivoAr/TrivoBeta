...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!otroUsuarioId) {
      return new Response(JSON.stringify({ error: "Falta otroUsuarioId" }), {
        status: 400,
      });
    }

    if (!mongoose.isValidObjectId(otroUsuarioId)) {
      return new Response(JSON.stringify({ error: "otroUsuarioId inválido" }), {
        status: 400,
      });
    }
    ...
    const salidasSocialesComunes = miembrosSalidasUsuario1
      .filter((m1) =>
        miembrosSalidasUsuario2.some(
          (m2) => m1.salida_id.toString() === m2.salida_id.toString()
        )
      )
      .map((m) => m.salida_id);
    ...
    const salidasTeamComunes = miembrosTeamUsuario1
      .filter((m1) =>
        miembrosTeamUsuario2.some(
          (m2) => m1.teamsocial_id.toString() === m2.teamsocial_id.toString()
        )
      )
    ...
    const salidasSocialesDetalles = await SalidaSocial.find({
      ...
      .select("titulo fecha ubicacion imagen")
      .sort({ fecha: -1 });
    ...
    const totalComunes =
      salidasSocialesDetalles.length + salidasTeamDetalles.length;
