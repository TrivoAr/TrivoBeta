...
export async function POST(request: NextRequest) {
  try {
    ...
    const precio = parseFloat(salida.precio.toString().replace(/[^\d.,]/g, '').replace(',', '.'));
    if (isNaN(precio) || precio <= 0) {
      console.error("Precio inválido:", salida.precio, "Convertido a:", precio);
      return NextResponse.json({
        error: "Precio inválido",
        details: process.env.NODE_ENV === "development" ? { original: salida.precio, converted: precio } : undefined
      }, { status: 400 });
    }
    ...
    if (error?.response?.data) {
      console.error("MercadoPago API Error:", JSON.stringify(error.response.data, null, 2));
      return NextResponse.json({
